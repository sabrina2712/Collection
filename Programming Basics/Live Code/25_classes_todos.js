...
class TodoList {
  ...
  mapOverUser(callback) {
    return this.user.map(callback);
  }

  userWithMostTodos() {
    const userId = this.userIdWithMostTodos();
  ...
  userIdWithMostTodos() {
    const userStatistic = this.mostTodosBy("userId");
...
const todoList = new TodoList(users, todos);
console.log(todoList.userWithMostTodos());
console.log(todoList.getUserById(4));
